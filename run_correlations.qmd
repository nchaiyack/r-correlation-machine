...
```{r}
.normalize_directionality <- function(directionality, predictor_vars) {
  stopifnot(is.character(predictor_vars), length(predictor_vars) >= 0)

  # Start with defaults for every predictor
  dir <- stats::setNames(rep("two.sided", length(predictor_vars)), predictor_vars)
...
Main driving function (help from ChatGPT in R techniques - https://chatgpt.com/share/6942f1b9-9744-800f-8f6f-9021bca82c04).
...
.compute_correlations <- function(
  ...
  res <- d %>%
    ...
    dplyr::mutate(
      cor = dplyr::if_else(
        .data$n_pair >= min_n,
        suppressWarnings(stats::cor(.data$x, .data[[outcome]], method = .data$method[1], use = use)),
        NA_real_
      ),
      p_value = dplyr::if_else(
        .data$n_pair >= min_n,
        suppressWarnings(stats::cor.test(.data$x, .data[[outcome]], method = .data$method[1])$p.value),
        NA_real_
      ),
      .groups = "drop"
    ) %>%
...
run_correlations <- function(
  ...
  # Test parameters
  directionality = NULL, # (default, bidirectional)
  ...
  dir_map <- .normalize_directionality(directionality, predictor_vars = pred_names)
